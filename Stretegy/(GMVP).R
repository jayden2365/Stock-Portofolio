portfolio_fun <- function(data) {
  ...
  # create function for portfolio
  port <- function(Y) {
    ones <- rep(1, nrow(Y))
    Sigma_inv_1 <- solve(Y, ones)  #same as: inv(Sigma) %*% ones
    weight <- (1/as.numeric(ones %*% Sigma_inv_1)) * Sigma_inv_1
    return(weight)
  }
